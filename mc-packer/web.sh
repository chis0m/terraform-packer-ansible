...
sudo openssl req -newkey rsa:2048 -nodes -keyout /etc/pki/tls/private/mc.key -x509 -days 365 -out /etc/pki/tls/certs/mc.crt \
-subj "/C=NG/ST=Lagos/L=Lagos/O=acellware/OU=devops/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"


sudo sed -i 's/localhost.crt/mc.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/mc.key/g'  /etc/httpd/conf.d/ssl.conf


# ssh timeout resolution
